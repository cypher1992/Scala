...
class RecursiveTest extends AnyFlatSpec with Matchers {
  ...
  "ArtofScala Recusion: multiplyItSelfCon(2)" should "return 2" in {
    val recur:Recursion = new Recursion()
    val actual:Int = recur.multiplyItSelfCon(2)
    val expected:Int = 2
    assert(actual == expected)
  }

  "ArtofScala Recusion: multiplyItSelfCon(5)" should "return 120" in {
    val recur:Recursion = new Recursion()
    val actual:Int = recur.multiplyItSelfCon(5)
    val expected:Int = 120
    assert(actual == expected)
  }
